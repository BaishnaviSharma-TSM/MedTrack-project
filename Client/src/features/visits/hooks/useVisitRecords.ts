...
import type { DateRangeChip, VisitRecord } from '../types';
import { getVisitRecords } from '../services/visitRecordService';
...
export function useVisitRecords(initialDateRange: DateRangeChip = '30d') {
  ...
  const refresh = useCallback(async (query = searchQuery) => {
    setIsLoading(true);
    try {
      const data = await getVisitRecords(query, buildQueryParams(dateRange, fromDate, toDate));
      setRecords(data);
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredRecords = useMemo(() => {
    return records.filter((record) => {
      if (fromDate || toDate) {
        return isWithinRange(record.visit.visitDate, fromDate, toDate);
      }

      if (dateRange === '7d') return isWithinDays(record.visit.visitDate, 7);
      if (dateRange === '30d') return isWithinDays(record.visit.visitDate, 30);
      return true;
    });
